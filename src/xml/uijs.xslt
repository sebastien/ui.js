...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:do="https://github.com/sebastien/uijs#do" xmlns:ui="https://github.com/sebastien/uijs" xmlns:on="https://github.com/sebastien/uijs#on" xmlns:out="https://github.com/sebastien/uijs#out" xmlns:s="https://github.com/sebastien/uijs#s" xmlns:x="https://github.com/sebastien/uijs#x" version="1.0">
			...
			<head>
				...
				<script type="importmap">
					...
					<xsl:for-each select="/*/ui:import[@module!='']">
						<xsl:text>"</xsl:text><xsl:value-of select="@module"/><xsl:text>/": "</xsl:text><xsl:choose><xsl:when test="@path"><xsl:value-of select="@path"/>/</xsl:when><xsl:otherwise>lib/js/<xsl:value-of select="substring-after(@module,'@')"/>/</xsl:otherwise></xsl:choose><xsl:text>",</xsl:text>
					</xsl:for-each>
				...
				<xsl:if test="//*[starts-with(name(),'x:')]">
					...
					"../components/<xsl:value-of select="local-name()"/>.xml"</xsl:for-each>
						<xsl:text>])]);</xsl:text>
					</script>
				</xsl:if>
	...
	<xsl:template name="uijs-script">
		...
		<script type="module" data-skip="true">
			...
			<xsl:text>// We instanciate the components in scope with the data&#10;const scope=(document.getElementById("</xsl:text><xsl:value-of select="$scope" /><xsl:text>")||document);&#10;</xsl:text>
			<xsl:text>ui(scope, {</xsl:text><xsl:value-of select="$cid"/><xsl:text>:data}, {}, {allowDuplicateTemplates:false});</xsl:text>
		</script>
		<xsl:if test="//ui:Component[@mode!='simple']">
			<script type="module" data-skip="true"><![CDATA[
